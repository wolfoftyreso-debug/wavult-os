...
router.get("/api/processes/performance", async (req: Request, res: Response) => {
  try {
    ...
    if (error) {
      console.warn("process_executions query error:", error.message);
      // Return empty metrics rather than failing hard — table may not exist yet
      return res.json([]);
    }
    ...
    // Non-fatal: no NC table means zero NCs
    if (ncErr) {
      console.warn("non_conformances query error:", ncErr.message);
    }
    ...
    if (procErr) {
      console.warn("processes query error:", procErr.message);
    }
