...
```bash
mkdir -p $(dirname .omc/state/team/g-r-en-fullst-ndig-audit-och-u/workers/worker-5/.ready) && touch .omc/state/team/g-r-en-fullst-ndig-audit-och-u/workers/worker-5/.ready
```
...
1. **Claim** your task (run this command first):
   `omc team api claim-task --input "{\"team_name\":\"g-r-en-fullst-ndig-audit-och-u\",\"task_id\":\"<id>\",\"worker\":\"worker-5\"}" --json`
...
3. **Send ACK** to the leader:
   `omc team api send-message --input "{\"team_name\":\"g-r-en-fullst-ndig-audit-och-u\",\"from_worker\":\"worker-5\",\"to_worker\":\"leader-fixed\",\"body\":\"ACK: worker-5 initialized\"}" --json`
...
- **Worker**: worker-5
- **Agent Type**: claude
- **Environment**: OMC_TEAM_WORKER=g-r-en-fullst-ndig-audit-och-u/worker-5
...
- Claim task: `omc team api claim-task --input "{\"team_name\":\"g-r-en-fullst-ndig-audit-och-u\",\"task_id\":\"<id>\",\"worker\":\"worker-5\"}" --json`
...
- Release claim (rollback): `omc team api release-task-claim --input "{\"team_name\":\"g-r-en-fullst-ndig-audit-och-u\",\"task_id\":\"<id>\",\"claim_token\":\"<claim_token>\",\"worker\":\"worker-5\"}" --json`

## Communication Protocol
- **Inbox**: Read .omc/state/team/g-r-en-fullst-ndig-audit-och-u/workers/worker-5/inbox.md for new instructions
- **Status**: Write to .omc/state/team/g-r-en-fullst-ndig-audit-och-u/workers/worker-5/status.json:
  ```json
  {"state": "idle", "updated_at": "<ISO timestamp>"}
  ```
  States: "idle" | "working" | "blocked" | "done" | "failed"
- **Heartbeat**: Update .omc/state/team/g-r-en-fullst-ndig-audit-och-u/workers/worker-5/heartbeat.json every few minutes:
  ```json
...
- To leader: `omc team api send-message --input "{\"team_name\":\"g-r-en-fullst-ndig-audit-och-u\",\"from_worker\":\"worker-5\",\"to_worker\":\"leader-fixed\",\"body\":\"<message>\"}" --json`
- Check mailbox: `omc team api mailbox-list --input "{\"team_name\":\"g-r-en-fullst-ndig-audit-och-u\",\"worker\":\"worker-5\"}" --json`
- Mark delivered: `omc team api mailbox-mark-delivered --input "{\"team_name\":\"g-r-en-fullst-ndig-audit-och-u\",\"worker\":\"worker-5\",\"message_id\":\"<id>\"}" --json`
...
1. Write your decision to: .omc/state/team/g-r-en-fullst-ndig-audit-och-u/workers/worker-5/shutdown-ack.json
2. Format:
