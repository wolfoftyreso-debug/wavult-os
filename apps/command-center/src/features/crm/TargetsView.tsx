    ...
    <div className="bg-surface-raised border border-surface-border rounded-xl p-5 flex flex-col gap-4">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="font-bold text-white">{member.member}</p>
          ...
          <div key={i} className="space-y-1.5">
            <div className="flex items-center justify-between text-xs">
              ...
              <span className="text-white font-semibold tabular-nums">
                ...
                <span className="text-gray-600 font-normal"> / {t.unit === 'SEK' ? formatSEK(t.goal) : `${t.goal} ${t.unit}`}</span>
              </span>
            </div>
    ...
    <div className="space-y-6">
      ...
      >
        ...
        <div className="text-center flex-shrink-0">
          ...
          <p className="text-sm text-gray-400">dagar kvar</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
        <div className="bg-surface-raised border border-surface-border rounded-xl px-5 py-4 text-center">
          <p className="text-3xl font-bold text-white">{totalSignedCustomers}</p>
          <p className="text-sm text-gray-400 mt-1">Signerade kunder</p>
          <ProgressBar current={totalSignedCustomers} goal={8} color="#10B981" />
          <p className="text-xs text-gray-600 mt-1">Mål: 8</p>
        </div>
        <div className="bg-surface-raised border border-surface-border rounded-xl px-5 py-4 text-center">
          <p className="text-2xl font-bold text-white">{formatSEK(totalSignedARR)}</p>
          <p className="text-sm text-gray-400 mt-1">Total signerad ARR</p>
          <ProgressBar current={totalSignedARR} goal={totalGoalARR} color="#3B82F6" />
          <p className="text-xs text-gray-600 mt-1">Mål: {formatSEK(totalGoalARR)}</p>
        </div>
        <div className="bg-surface-raised border border-surface-border rounded-xl px-5 py-4 text-center">
          <p className="text-3xl font-bold text-white">{totalDemos}</p>
          <p className="text-sm text-gray-400 mt-1">Demos utförda</p>
          <ProgressBar current={totalDemos} goal={15} color="#8B5CF6" />
          <p className="text-xs text-gray-600 mt-1">Mål: 15</p>
        </div>
      </div>
